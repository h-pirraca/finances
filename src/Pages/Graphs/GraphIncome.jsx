...
import TableMediaRendimento from '../../Components/Tables/TableMediaRendimento';
...
const GraphIncome = () => {
  ...
  const fetchData = async (username, index, label) => {
    ...
    try {
      ...
      const response = await fetch(process.env.REACT_APP_SERVER_LINK+'/graphIncome', {
        method: 'POST',
        headers: {
    ...
    <div className={GraphCSS.Top}>
      <div className={GraphCSS.Main}>
        <div className={GraphCSS.Article}>
          <h3>Gráfico Anual de Rendimentos</h3>
          <DatePicker
      ...
      <h5><b>Valor Médio Mensal</b></h5>
        <TableMediaRendimento valor={total.datasets} user={'Utilizador'} total={'Total'}/>
      </div>
    </div>
  );
};
